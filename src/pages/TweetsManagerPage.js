...
export default function TweetsManagerPage() {
  ...
  // ---- Import UI state (RSS -> many tweets) ----
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(`${API_BASE}/api/tweets`, { params });
      setItems(res.data || []);
    } catch (e) {
      console.error(e);
      setError("Failed to load tweets. Is the backend running?");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (payload) => {
    await axios.post(`${API_BASE}/api/tweets`, payload);
    setEditing(null);
    await fetchData();
  };

  const handleUpdate = async (id, payload) => {
    await axios.put(`${API_BASE}/api/tweets/${id}`, payload);
    setEditing(null);
    await fetchData();
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Delete this tweet item?")) return;
    await axios.delete(`${API_BASE}/api/tweets/${id}`);
    await fetchData();
  };

  const handleToggleEnabled = async (id, enabled) => {
    await axios.put(`${API_BASE}/api/tweets/${id}`, { enabled });
    await fetchData();
  };
  ...
  const handleBumpSort = async (id, delta) => {
    ...
    await axios.put(`${API_BASE}/api/tweets/${id}`, { sortIndex: nextIndex });
    await fetchData();
  };

  // ---- Import from RSS (no deps) ----
  const doImport = async () => {
    try {
      ...
      const res = await axios.post(`${API_BASE}/api/tweets/import-simple`, payload);
  ...
  return (
    <div style={{ padding: 20, color: "#eaeaea", background: "#121212", minHeight: "100vh" }}>
      <h1 style={{ marginBottom: 12 }}>Tweets (X) Manager</h1>

      {/* Import from RSS */}
      <div style={{ marginBottom: 16, border: "1px solid #333", borderRadius: 8, padding: 12, background: "#0b1220" }}>
        <div style={{ fontWeight: 600, marginBottom: 8 }}>Import from RSS (handles/hashtags feeds)</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <input
            type="text"
            value={rssUrl}
            onChange={(e) => setRssUrl(e.target.value)}
            placeholder="https://rss.app/feeds/... (tweets for a handle or hashtag)"
